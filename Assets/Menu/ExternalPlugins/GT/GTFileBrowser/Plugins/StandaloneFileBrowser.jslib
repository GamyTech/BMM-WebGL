var StandaloneFileBrowserWebGLPlugin = {
    ...
    UploadFile: function(gameObjectNamePtr, functionNamePtr/*, filter, multiselect*/) {
        gameObjectName = Pointer_stringify(gameObjectNamePtr);
		functionName = Pointer_stringify(functionNamePtr);
        ...
        fileInput.onchange = function (event) {
			...
			console.log(FileUrl);
            SendMessage(gameObjectName, functionName, FileUrl);

            // Remove after file selected
            document.body.removeChild(document.getElementById(gameObjectName));
        }
    ...
    UploadFolder: function(gameObjectNamePtr, functionNamePtr) {
        gameObjectName = Pointer_stringify(gameObjectNamePtr);
		functionName = Pointer_stringify(functionNamePtr);
        SendMessage(gameObjectName, functionName, '');
    },
    ...
    DownloadFile: function(gameObjectNamePtr, functionNamePtr, filenamePtr, byteArray, byteArraySize) {
        gameObjectName = Pointer_stringify(gameObjectNamePtr);
		functionName = Pointer_stringify(functionNamePtr);
