	...
	public partial class Localize : MonoBehaviour 
	{
		...
		public void OnLocalize( bool Force = false )
		{
            ...
            if (string.IsNullOrEmpty(FinalTerm) || string.IsNullOrEmpty(FinalSecondaryTerm))
				GetFinalTerms( out FinalTerm, out FinalSecondaryTerm, true);
		...
		public void GetFinalTerms( out string PrimaryTerm, out string SecondaryTerm, bool logErrors)
		{
            ...
            if(logErrors && string.IsNullOrEmpty(mTerm))
            {
                if(mTarget_uGUI_Text == null)
                    Debug.Log("No Target to Localize in " + Utils.ListParentChain(gameObject));
                else
                    Debug.LogError("No assigned term: " + mTarget_uGUI_Text.text + " to localize in " + Utils.ListParentChain(gameObject) + "   ;)");
            }
		...
		{
			PrimaryTerm = RemoveNonASCII ? 	System.Text.RegularExpressions.Regex.Replace(Main, @"[^a-zA-Z0-9_ ]+", " ") : Main;
